    ...
    function renderFilterCard() {
        ...
        if (m.mode === 'multi') {
            ...
            $wrap.off('change.multif').on('change.multif','label.fc-chain input, label.fc-cex input, label.fc-dex input',function(){
                ...
                let dex=$wrap.find('label.fc-dex input:checked').map(function(){return $(this).closest('label').attr('data-val').toLowerCase();}).get();

                // Enforce max 4 DEX selected in MULTICHAIN
                try {
                    const isDexToggle = $(this).closest('label').hasClass('fc-dex');
                    const isChecking = $(this).is(':checked');
                    if (isDexToggle && isChecking && dex.length > 4) {
                        // revert this toggle
                        $(this).prop('checked', false);
                        dex = $wrap.find('label.fc-dex input:checked').map(function(){return $(this).closest('label').attr('data-val').toLowerCase();}).get();
                        const warn = 'Maksimal 4 DEX dapat dipilih untuk scanning.';
                        if (typeof toast !== 'undefined' && toast.warning) toast.warning(warn); else alert(warn);
                    }
                } catch(_) {}
            ...
            $wrap.off('change.scf').on('change.scf','label.sc-cex input, label.sc-pair input, label.sc-dex input',function(){
                ...
                let d=$wrap.find('label.sc-dex input:checked').map(function(){return $(this).closest('label').attr('data-val').toLowerCase();}).get();

                // Enforce max 4 DEX selected in SINGLE-CHAIN
                try {
                    const isDexToggle = $(this).closest('label').hasClass('sc-dex');
                    const isChecking = $(this).is(':checked');
                    if (isDexToggle && isChecking && d.length > 4) {
                        $(this).prop('checked', false);
                        d = $wrap.find('label.sc-dex input:checked').map(function(){return $(this).closest('label').attr('data-val').toLowerCase();}).get();
                        const warn = 'Maksimal 4 DEX dapat dipilih untuk scanning.';
                        if (typeof toast !== 'undefined' && toast.warning) toast.warning(warn); else alert(warn);
                    }
                } catch(_) {}
...
$(document).ready(function() {
    ...
    // If another tab holds RUN_LOCK, gate Start button and show info
    try {
        const lock = (typeof getRunLock === 'function') ? getRunLock() : null;
        if (lock && (typeof isRunLockActive === 'function') ? isRunLockActive(lock) : false) {
            const myId = (typeof getTabId === 'function') ? getTabId() : '';
            if (String(lock.ownerTabId) !== String(myId)) {
                // Gate UI similar to running state, but do not claim run locally
                $('#startSCAN').prop('disabled', true).addClass('uk-button-disabled').text('Running...').attr('aria-busy','true');
                $('#stopSCAN').hide().prop('disabled', true);
                const modeTxt = (lock.mode === 'single') ? `CHAIN: ${(lock.chain||'').toString().toUpperCase()}` : 'MULTICHAIN';
                $('#infoAPP').html(` RUN SCANNING (TAB LAIN): ${modeTxt}`).show();
            }
        }
    } catch(_) {}
    ...
    function handleRunLock(lockVal){
        try {
            const active = (typeof isRunLockActive === 'function') ? isRunLockActive(lockVal) : !!lockVal;
            const myId = (typeof getTabId === 'function') ? getTabId() : '';
            const owned = !!(lockVal && String(lockVal.ownerTabId) === String(myId));
            if (active && !owned) {
                $('#startSCAN').prop('disabled', true).addClass('uk-button-disabled').text('Running...').attr('aria-busy','true');
                $('#stopSCAN').hide().prop('disabled', true);
                const modeTxt = (lockVal && lockVal.mode === 'single') ? `CHAIN: ${(lockVal.chain||'').toString().toUpperCase()}` : 'MULTICHAIN';
                $('#infoAPP').html(` RUN SCANNING (TAB LAIN): ${modeTxt}`).show();
            } else {
                // Only relax if this tab is not actively running
                const st = (typeof getAppState === 'function') ? getAppState() : { run: 'NO' };
                if (!st || String(st.run).toUpperCase() !== 'YES') {
                    $('#startSCAN').prop('disabled', false).removeClass('uk-button-disabled').text('Start').removeAttr('aria-busy');
                    $('#stopSCAN').hide().prop('disabled', true);
                    if (!active) { try { $('#infoAPP').text('').hide(); } catch(_) {}
                    }
                }
            }
        } catch(_) {}
        ...
        window.__MC_BC.addEventListener('message', function(ev){
            ...
            if (msg.type === 'kv') {
                try {
                    ...
                    if (keyUpper === 'RUN_LOCK') { handleRunLock(msg.val); return; }
    ...
    // Build chain mode dropdown (MULTICHAIN + per-chain list)
    function renderChainLinks(activeKey = 'all') {
        const $sel = $('#chain-mode-select');
        if ($sel.length === 0) return;
        $sel.empty();

        // Add MULTICHAIN option
        $sel.append('<option value="all">MULTICHAIN</option>');

        // Add chains from CONFIG_CHAINS
        const keys = Object.keys(CONFIG_CHAINS || {}).sort();
        keys.forEach(chainKey => {
            const cfg = CONFIG_CHAINS[chainKey] || {};
            const label = (cfg.Nama_Chain || chainKey).toString().toUpperCase();
            $sel.append(`<option value="${chainKey}">${label}</option>`);
        });

        const want = String(activeKey || 'all').toLowerCase();
        const valid = want === 'all' || keys.map(k => k.toLowerCase()).includes(want);
        $sel.val(valid ? want : 'all');
    }

    // Handle dropdown change → navigate to selected mode
    $(document).on('change', '#chain-mode-select', function(){
        try {
            const val = String($(this).val() || 'all');
            const currentPage = (window.location.pathname.split('/').pop() || 'index.html');
            window.location.href = `${currentPage}?chain=${encodeURIComponent(val)}`;
        } catch(_) {}
    });
...
// Release RUN_LOCK if this tab owns it when closing/navigating away
try {
  window.addEventListener('beforeunload', function(){
    try { if (typeof releaseRunLock === 'function') releaseRunLock(); } catch(_) {}
  });
} catch(_){}
