...
async function startScanner(tokensToScan, settings, tableBodyId) {
    ...
    try {
        ...
        const resLock = (typeof acquireRunLock === 'function') ? acquireRunLock(mLock) : { ok:true };
        if (!resLock || resLock.ok === false) {
            const holder = resLock.lock || {};
            const modeTxt = (holder.mode === 'single') ? `CHAIN: ${(holder.chain||'').toString().toUpperCase()}` : 'MULTICHAIN';
            const msg = `SCAN sedang berjalan di tab lain (${modeTxt}).`;
            if (typeof toast !== 'undefined' && toast.warning) toast.warning(msg); else alert(msg);
        ...
        // Start heartbeat to keep lock alive during scanning
        try { clearInterval(window.__RUN_LOCK_HEARTBEAT); } catch(_) {}
        window.__RUN_LOCK_HEARTBEAT = setInterval(function(){ try { if (typeof renewRunLock === 'function') renewRunLock(); } catch(_){} }, 20000);
    } catch(_) {}
