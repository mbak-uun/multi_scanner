...
async function startScanner(tokensToScan, settings, tableBodyId) {
    ...
    function processUiUpdates() {
        ...
        const batch = uiUpdateQueue.splice(0, 10);
        batch.forEach(updateData => {
            if (updateData.type === 'error') {
                const { id, color, message, swapMessage } = updateData;
                const cell = document.getElementById(id);
                if (cell) {
                    cell.style.backgroundColor = color || '#f39999';
                    const dexNameAndModal = cell.querySelector('strong')?.outerHTML || '';
                    cell.innerHTML = `
                        ${dexNameAndModal}<br>
                        <span class="uk-text-danger" title="${message || 'Unknown Error'}">${swapMessage || '[ERROR]'}</span>
                    `;
                }
            } else {
                ...
                token.dexs.forEach((dexData) => {
                            ...
                            const callDex = (direction) => {
                                ...
                                const updateDexCellStatus = (status, dexName, message = '') => {
                                    ...
                                    switch (status) {
                                        case 'checking': statusHtml = `<span class="uk-text-muted">Checking ${dexName.toUpperCase()}...</span>`; break;
                                        case 'fallback': statusHtml = `<span class="uk-text-warning" title="Initial Error: ${message}">Scanning SWOOP...</span>`; break;
                                        case 'error':
                                        case 'fallback_error':
                                            cell.style.backgroundColor = '#ffcccc';
                                            statusHtml = `<span class="uk-text-danger" title="${message}">[ERROR]</span>`;
                                            break;
                                    }
                                    cell.innerHTML = `${dexNameAndModal}<br>${statusHtml}`;
                                };
                                ...
                                const handleSuccess = (dexResponse, isFallback = false) => {
                                    ...
                                    const update = calculateResult(
                                        baseId, finalDexRes.tableBodyId, finalDexRes.amount_out, finalDexRes.FeeSwap,
