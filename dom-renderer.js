...
function InfoSinyal(DEXPLUS, TokenPair, PNL, totalFee, cex, NameToken, NamePair, profitLossPercent, modal, nameChain, codeChain, trx, idPrefix, domIdOverride) {
  // Normalisasi nama DEX agar konsisten dengan id kartu yang dibuat RenderCardSignal()
  // Beberapa layanan mengembalikan label seperti 'FLYTRADE' atau 'KYBERSWAP'.
  const rawKey = String(DEXPLUS || '').toLowerCase();
  const dexAliasMap = { 'flytrade': 'fly', 'kyberswap': 'kyber', 'zero-kyber': 'kyber' };
  const dexKey = dexAliasMap[rawKey] || rawKey;
