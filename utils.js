...
// =================================================================================
// GLOBAL RUN LOCK (mutex) TO ENFORCE SINGLE ACTIVE SCAN ACROSS TABS
// =================================================================================
(function initRunLockHelpers(){
  ...
  function getTabId(){
    try {
      ...
      // Fallback: ephemeral id per call
      return (Date.now().toString(36) + Math.random().toString(36).slice(2,10));
    }
  }

  function getRunLock(){
    try { return getFromLocalStorage(RUN_LOCK_KEY, null); } catch(_) { return null; }
  }

  function isRunLockActive(lock){
    try {
      if (!lock || typeof lock !== 'object') return false;
      const startedAt = parseInt(lock.startedAt || 0);
      const ttl = parseInt(lock.ttlMs || DEFAULT_TTL);
      if (!Number.isFinite(startedAt) || startedAt <= 0) return false;
      return (Date.now() - startedAt) < Math.max(15000, ttl);
  ...
  function acquireRunLock(mode){
    try {
      ...
      const existing = getRunLock();
      if (isRunLockActive(existing) && String(existing.ownerTabId) !== String(owner)) {
        return { ok:false, lock: existing };
      }
      const m = (typeof getAppMode === 'function') ? getAppMode() : { type: 'multi' };
      const payload = {
        ownerTabId: owner,
        mode: (mode && mode.type) ? mode.type : (m.type || 'multi'),
        chain: (mode && mode.chain) ? mode.chain : (m.chain || null),
        startedAt: Date.now(),
        ttlMs: DEFAULT_TTL
      };
      saveToLocalStorage(RUN_LOCK_KEY, payload);
      try { window.__RUN_LOCK_OWNED = true; } catch(_) {}
      return { ok:true, lock: payload };
    } catch(e) { return { ok:false, error: e }; }
  }

  function renewRunLock(){
    try {
      const cur = getRunLock();
      const owner = getTabId();
      if (!cur || String(cur.ownerTabId) !== String(owner)) return false;
      cur.startedAt = Date.now();
      saveToLocalStorage(RUN_LOCK_KEY, cur);
      return true;
    } catch(_) { return false; }
  }
  ...
  function releaseRunLock(){
    try {
      const cur = getRunLock();
      const owner = getTabId();
      if (cur && String(cur.ownerTabId) === String(owner)) {
        try { removeFromLocalStorage(RUN_LOCK_KEY); } catch(_) { saveToLocalStorage(RUN_LOCK_KEY, null); }
        try { window.__RUN_LOCK_OWNED = false; } catch(_) {}
        return true;
      }
      return false;
    } catch(_) { return false; }
  ...
  try {
    if (typeof window !== 'undefined'){
      window.getTabId = window.getTabId || getTabId;
      window.getRunLock = window.getRunLock || getRunLock;
      window.isRunLockActive = window.isRunLockActive || isRunLockActive;
      window.acquireRunLock = window.acquireRunLock || acquireRunLock;
      window.renewRunLock = window.renewRunLock || renewRunLock;
      window.releaseRunLock = window.releaseRunLock || releaseRunLock;
    }
  } catch(_){}
})();
        ...
        window.resolveActiveDexList = window.resolveActiveDexList || function resolveActiveDexList(){
            try {
                ...
                if (m.type === 'single') {
                    ...
                    const norm = (list || []).map(x => String(x).toLowerCase());
                    return norm.slice(0, 4); // limit to 4 DEX for scanner/cards
                } else {
...
function generateDexLink(dex, chainName, codeChain, NameToken, sc_input, NamePair, sc_output) {
    ...
    if (!dexKey) {
        const synonyms = { kyberswap: 'kyber', fly: 'flytrade' };
